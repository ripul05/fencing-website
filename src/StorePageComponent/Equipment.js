...
function EquipmentCard({ item, index, loaded, onRequestService }) {
...
export default function EquipmentStore() {
  ...
  // Fetch equipment data from Sanity
  useEffect(() => {
    const fetchEquipment = async () => {
      try {
        setLoading(true);
        const query = `
          *[_type == "equipment" && isActive == true] | order(sortOrder asc) {
            _id,
            equipmentId,
            title,
            price,
        ...
        const equipment = await sanityFetch(query);
  ...
  const handleRequestService = (item) => {
    if (item.paymentLink) {
      ...
      const href = `${item.paymentLink}${item.trackingQuery ? (item.paymentLink.includes("?") ? "&" : "?") + item.trackingQuery : ""}`;
      window.open(href, '_blank');
    } else {
      console.log('Equipment requested for:', item);
      // Handle custom booking/request logic
    }
  };

  // Get unique categories from fetched data
  const categories = ["All Equipment", ...new Set(equipmentData.map(item => item.category))];
      ...
      <div className="min-h-screen bg-gradient-to-b from-slate-50 to-slate-100">
        ...
        <div className="flex items-center justify-center py-20">
          <div className="text-center">
            ...
            <p className="text-slate-600 text-lg">Loading equipment...</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="min-h-screen bg-gradient-to-b from-slate-50 to-slate-100">
        ...
        <div className="flex items-center justify-center py-20">
          <div className="text-center max-w-md mx-auto px-4">
            ...
            <h3 className="text-xl font-semibold text-slate-800 mb-2">Unable to Load Equipment</h3>
